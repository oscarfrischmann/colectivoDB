...
if (seminariosForm) {
	seminariosForm.addEventListener('submit', async (e) => {
		...
		try {
			...
			await updateDoc(doc(db, 'seminarios', 'seminario'), seminario, { merge: true });
...
showImages('seminario', 'seminarioImgContainer', 'showEminarioImages', 'seminarioImg', seminariosForm);
